                ...
                )
{
	...
	vPosProj = float4(-vPosProj.x*Time + invTime*(vTexCoordIn.x-.5)*aspectInv*2.0, vPosProj.y*Time + invTime*(vTexCoordIn.y-.5)*2.0, vPosProj.z * Time, vPosProj.w*Time + invTime);

	vColorOut.rgb = vNormalObject;
	vTexCoordOut = vTexCoordIn;
	vTexCoordOut.y = -vTexCoordOut.y;	
	vTexCoordOut.y /=aspectInv;

	//vTexCoordOut.y += .61166;
	//vTexCoordOut.x *=.7;
	//vTexCoordOut.y *=.7;
