                ...
                )
{
    ...
    // for display
    float4 vPosWorld = mul( vPosObject, World );
    vPosProj = mul( vPosObject, WorldViewProjection );
    
    // Transform the normal into world space for lighting
    float3 vNormalWorld = mul( vNormalObject, (float3x3)World );
    
    // Pass the texture coordinate
    vTexCoordOut = vTexCoordIn;
    vTexCoordOut.y = 1-vTexCoordOut.y;
...
{  
    vColorOut = vColorIn;
    //vColorOut = float4(1,1,1,1);

//    float4 temp=tex2D( MeshTextureSampler, vTexCoord ).rgba;
//    float3 normal=2.0*temp.rgb-float3(1,1,1);
...
//	{	
//		vColorOut.rgb *= size ;
//		//vColorOut.rgb = float3(0,0,0);
//		
//		vColorOut.rgb *= .5 * (1.0+dot(normal, float3(.71,-.71,0)));
//		//vColorOut.rgb *= dot(normal, float3(.71,-.71,0));
//		vColorOut.r += .1 * (1.0+dot(normal, float3(0,0,1)));
//		vColorOut.g += size * .1 * (.3+dot(normal, float3(0,-1,0)));
//		vColorOut.b += size * .7 * (.3+dot(normal, float3(0,1,0)));
//	}
}
