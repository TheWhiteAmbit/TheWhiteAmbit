...
namespace TheWhiteAmbit {
	...
	void
		VideoDecoder::mapFrame(int iPictureIndex, void* ppDevice, unsigned int* pPitch, CUVIDPROCPARAMS * pVideoProcessingParameters)
	{
		...
		assert(CUDA_SUCCESS == oResult);
		assert(0 != *(unsigned int*)ppDevice);
		assert(0 != *pPitch);
	}

	void
		VideoDecoder::unmapFrame(void* pDevice)
	{
		CUresult oResult = cuvidUnmapVideoFrame(oDecoder_, *(unsigned int*)pDevice);
		assert(CUDA_SUCCESS == oResult);
	}
}