	...
	HRESULT CMeshLoader10::Create( ID3D10Device* pd3dDevice, const WCHAR* strFilename )
	{
		...
		for ( int iMaterial = 0; iMaterial < m_Materials.GetSize(); ++iMaterial )
		{
			...
			{            
				...
				{
					DXUTGetGlobalResourceCache().CreateTextureFromFile( pd3dDevice, pMaterial->strTexture, 
		...
		UINT numElements;
		D3D10_INPUT_ELEMENT_DESC* pDecl=Vertex::getVertexElements(&numElements);
		//TODO: test everything is right here, specially with 'pDecl[0].SemanticName'
		V_RETURN( D3DX10CreateMesh( pd3dDevice,
			pDecl,
			numElements,
			pDecl[0].SemanticName,
			m_Vertices.GetSize(),
			m_Indices.GetSize() / 3,
			D3DX10_MESH_32_BIT,
			&pMesh ) );
